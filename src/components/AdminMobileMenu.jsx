...
export default function AdminMobileMenu() {
  ...
  // Closes this bottom drawer first so it never sits stacked behind the
  // Action Required side panel (the same panel/toggle the desktop sidebar's
  // bell icon and the floating button use — see ActionRequiredCenter).
  const openActions = () => {
    setOpen(false);
    openActionRequired();
  };
          ...
          <div className="mt-3 grid gap-1.5">
            {nav.map((item) =>
              item.type === 'group' ? (
                ...
                item.type === 'action' ? (
                  <Row key={item.id} item={item} active={false} badge={actionItems.length} onClick={openActions} />
                ) : (

...
// Product selector for wheel "Product" rewards — searches the live catalog by
// name, barcode, or variant SKU. Stores the chosen product id + name on the
// reward so the spin can snapshot it later.
export default function WheelProductPicker({ value, productName, onSelect }) {
  ...
  const search = async (text) => {
    ...
    const all = await base44.entities.Product.list('-created_date', 100).catch(() => []);

-- CRITICAL security fix (pre-launch audit): bundle line items crossed the
-- server-authoritative pricing/stock boundary in two places.
--
-- PART A — secure_order(): when an item claimed is_bundle=true with a
-- bundle_id that did NOT resolve to a real, active bundle row, the pricing
-- branch was `if found then ... end if` with no matching `else` — so v_unit
-- (initialized from the client-submitted price) was left untouched and
-- silently accepted as the final, "secured" price. Live-reproduced during
-- this fix: an order with a fabricated bundle_id and a client price of 0.01
-- came back `secured: true, total: 0.01` with zero rejection.
--
-- Fix: reject the order outright (matching every other precondition failure
-- in this function) whenever a bundle line's bundle_id doesn't resolve to a
-- bundle that is currently real, active, and inside its date window (the
-- exact eligibility rule the storefront itself already uses —
-- src/lib/bundles.js's isBundleActive). Client price is never used as a
-- fallback for an unresolved/ineligible bundle.
--
-- Because Checkout.jsx previously ignored secure_order's return value
-- entirely (`try { await secureOrder(orderId); } catch { /* non-blocking */
-- }`), a rejection here would otherwise have no effect and checkout would
-- silently continue with the unverified client totals — worse, an order
-- mixing one fabricated bundle line with otherwise-legitimate real items
-- would skip server repricing for the WHOLE order, not just the bad line,
-- since this function returns as soon as it hits the bad line. Checkout.jsx
-- is updated in the same batch to treat an explicit `{success:false}` from
-- secure_order as a hard stop (mirroring how commit_order_stock's failure
-- is already handled), so this rejection actually blocks the order.
--
-- PART B — commit_order_stock(): the bundle stock-deduction step derived
-- which products/quantities to decrement from the order's own client-
-- submitted `bundle_items` array, not from the bundle's real definition.
-- Live-reproduced during this fix: a real, active test bundle whose
-- authoritative definition requires 2 units of its component product had
-- its order line's bundle_items tampered to claim quantity 0 — the
-- function still returned success and only decremented the component by 1
-- unit (an existing `greatest(1, ...)` floor meant for genuine zero-qty
-- data, not a security control, prevented the literal 0 but did not
-- prevent the under-decrement) instead of the correct 2.
--
-- Fix: derive the components/quantities to decrement by joining the
-- order's `bundle_id` against public.bundles.items (the authoritative,
-- admin-configured definition) instead of trusting the order's own
-- bundle_items. A bundle_id that doesn't resolve to a real bundle now
-- contributes zero rows here (nothing to fulfill), instead of trusting
...
begin
  ...
  -- Bundle components/quantities are derived from the bundle's own
  -- authoritative definition (public.bundles.items), never from the
  -- order's client-submitted bundle_items — see migration header.
  insert into _needed (product_id, variant_key, qty, name, variant_label)
  ...
  from jsonb_array_elements(v_order.items) it
  join public.bundles b on b.id = (it->>'bundle_id')::uuid
  cross join lateral jsonb_array_elements(coalesce(b.items, '[]'::jsonb)) bi
  where coalesce((it->>'is_bundle')::boolean, false) = true
    and (it->>'bundle_id') is not null
